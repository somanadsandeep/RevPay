...
public class MoneyRequestServiceImpl implements MoneyRequestService {
	...
	public void requestMoney(int fromUser, int toUser, double amount) throws Exception {
		...
		String sql = "INSERT INTO payment_requests (from_user, to_user, amount, status) "
				+ "VALUES (?, ?, ?, 'PENDING')";
	...
	public void viewRequests(int userId) throws Exception {
		...
		String sql = "SELECT request_id, from_user, amount, status " + "FROM payment_requests WHERE to_user=?";
		...
		while (rs.next()) {
			found = true;
			System.out.println("Request ID: " + rs.getInt("request_id") + " | From User: " + rs.getInt("from_user")
					+ " | Amount: " + rs.getDouble("amount") + " | Status: " + rs.getString("status"));
		}
	...
	public void respondToRequest(int requestId, boolean approve) throws Exception {
		...
		// 🔹 Get request details
		String fetchSql = "SELECT from_user, to_user, amount " + "FROM payment_requests "
		...
		if (approve) {
			...
			transactionDAO.addTransaction(toUser, fromUser, amount, "REQUEST_TRANSFER", "Money request approved");
		}

		// UPDATE REQUEST STATUS
		String updateSql = "UPDATE payment_requests SET status=? WHERE request_id=? AND status='PENDING'";
		...
		System.out.println(approve ? "Request APPROVED and money transferred" : "Request REJECTED");
	}
}